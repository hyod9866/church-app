...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventContent: (arg) => {
            ...
            return { html: `<div class="p-1 overflow-hidden"><div class="font-bold text-[13px] truncate">${arg.event.title}${(type === '설교' || type === '외부설교' || type === '교회행사') ? '' : ` (${count})`}</div>${subtext}</div>` };
        },
        dateClick: (info) => openMeetingModal(null, info.dateStr),
        eventClick: (info) => showMeetingDetail(info.event.id, info.event.startStr, info.event.title, info.event.extendedProps.type, info.event.extendedProps.sermon_title, info.event.extendedProps.memo, info.event.extendedProps.church)
    });
    calendar.render();
    ...
    function openAddModal(isEdit = false) {
        ...
        if (!isEdit) {
            ...
            const parishName = getSelectedParishName();
            const formParishSelect = memberAddForm.querySelector('select[name="parish"]');
            if (formParishSelect && parishName && parishName !== '교구 없음') {
                formParishSelect.value = parishName;
            }
        }
        else {

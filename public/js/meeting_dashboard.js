...
async function fetchAttendanceCharts() {
    try {
        ...
        const renderChart = (id, catData, targetMonths, targetKeys, isBar = true, kpiContainerId = null, alertContainerId = null, isStacked = false) => {
            ...
            //  - 각 구역의 달별 출석 평균만 표기 (모임이 없던 달은 제외 — 빨간 점선과 동일 기준)
            //  - "전체" 값은 차트의 빨간 점선(전체 평균)이 이미 보여주므로 괄호에는 넣지 않는다
            const avgLabelEl = document.getElementById(id + 'AvgLabel');
            if (avgLabelEl) {
                const parts = [];
                Object.keys(catData).sort().forEach(g => {
                    const vals = [];
                    targetKeys.forEach(mk => {
                        if (mk > currentMonthKey) return;
                ...
                avgLabelEl.textContent = parts.length > 0 ? `(월평균 - ${parts.join(' / ')})` : '';
            }
